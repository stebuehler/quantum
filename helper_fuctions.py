...
def drop_numerical_columns_and_return_one_row(df):
    numerical_columns = ['Position [m]', 'Speed [m/s]', 'Acceleration [m/(s^2)]', 'Force [N]', 'Sample duration [s]', 'time', 'v_rel', '#Row']
...
def process_single_file(full_df):
    ...
    for repetition in all_repetitons:
        ...
        df_result = add_result_values(df_result, time_total, times_to_v_x, time_to_a_max)
